<?php
	...
	********************************************/
	...
	//'------------------------------------
	$API_UserName=	'digvijay.daga.merchant_api1.imarkinfotech.com';
	$API_Password=	'278RE4MVNJVE2LLC';
	$API_Signature=	'AFcWxV21C7fd0v3bYYYRCpSSRl31AnjyVr7.iK-J4nzxW0pVYoY2xaoW';
	...
	*/
	function CallShortcutExpressCheckout( $paymentAmount, $currencyCodeType, $paymentType, $returnURL, $cancelURL) 
	{
		...
		$nvpstr = $nvpstr . "&BILLINGAGREEMENTDESCRIPTION=".$itemDesc;
	...
	*/
	...
	function CreateRecurringPaymentsProfile()
	{
		...
		$GetnextPayDate	=	 date("Y-m-1", strtotime(date('m', strtotime('+1 month')).'/01/'.date('Y').' 00:00:00'));
		$nextPayDate	=	 $GetnextPayDate."T0:0:0";
		$itemDesc 		= $_SESSION['itemDesc'];
	    //'---------------------------------------------------------------------------
		//' Build a second API request to PayPal, using the token as the
		//'  ID to get the details on the payment authorization
		...
		$nvpstr.="&PROFILESTARTDATE=".urlencode($nextPayDate);
		$nvpstr.="&DESC=".$itemDesc;
	...
	function ManageRecurringPaymentsProfileStatus()
	{
		//'--------------------------------------------------------------
		//' At this point, the buyer has completed authorizing the payment
		//' at PayPal.  The function will call PayPal to obtain the details
		...
		$token 			= urlencode($_SESSION['TOKEN']);
		$email 			= urlencode($_SESSION['email']);
		$Get_PROFILEID 	=  urlencode($_SESSION["PROFILEID"]);
	    //'---------------------------------------------------------------------------
		//' Build a second API request to PayPal, using the token as the
		//'  ID to get the details on the payment authorization
		//'---------------------------------------------------------------------------
		$nvpstr="&PROFILEID=".$Get_PROFILEID;
